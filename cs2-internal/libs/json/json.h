...
namespace C::JSON
{
	...
	inline void WriteBuffer(nlohmann::json& entry, const VariableObject_t& variable)
	{
		...
		default:
		{
			...
			for (const UserDataType_t& userType : vecUserTypes)
			{
				...
				{
					// write size
					entry[CXOR("size")] = variable.GetSerializationSize();
					...
					for (const UserDataMember_t& userMember : userType.vecMembers)
					{
						...
						currentMember[CXOR("name")] = userMember.uNameHash;
						currentMember[CXOR("type")] = userMember.uTypeHash;
	...
	inline void ReadBuffer(nlohmann::json& entry, VariableObject_t& variable)
	{
		...
		default:
		{
			...
			for (const UserDataType_t& userType : vecUserTypes)
			{
				...
				{
					// read size
					const std::size_t nSourceSerializationSize = entry[CXOR("size")].get<std::size_t>();

					nlohmann::json members = entry[CXOR("members")];
					...
					for (const UserDataMember_t& userMember : userType.vecMembers)
					{
						const FNV1A_t uMemberNameHash = members[CXOR("name")].get<FNV1A_t>();
						const FNV1A_t uMemberTypeHash = members[CXOR("type")].get<FNV1A_t>();
	...
	inline bool SaveVariable(const wchar_t* wszFilePath, const VariableObject_t& variable)
	{
		...
		::_wfopen_s(&hFileStream, wszFilePath, CXOR(L"r"));
	...
	inline bool LoadFile(const wchar_t* wszFilePath)
	{
		...
		//		L_PRINT(LOG_WARNING) << CXOR("failed to re-serialize configuration file: \"") << wszFileName << CXOR("\"");
		//
		//	::CloseHandle(hFileOut);
		//}
