...
{
	constexpr TextOverlayVar_t(const bool bEnable, const bool bIcon, const float flThickness = 1.f, const Color_t& colPrimary = Color_t(255, 255, 255), const Color_t& colOutline = Color_t(0, 0, 0, 0)) :
		bEnable(bEnable), bIcon(bIcon), colPrimary(colPrimary), colOutline(colOutline) { }

	bool bEnable = false;
	bool bIcon = false;

	Color_t colPrimary = Color_t(255, 255, 255);
	Color_t colOutline = Color_t(0, 0, 0, 0);
};
...
{
	constexpr BarOverlayVar_t(const bool bEnable, const bool bUseFactorColor = false, const float flThickness = 1.f, const Color_t& colSecondary = Color_t(255, 255, 255), const Color_t& colBackground = Color_t(), const Color_t& colOutline = Color_t(), const bool background = true, const bool outline = true) :
		bEnable(bEnable), bUseFactorColor(bUseFactorColor), flThickness(flThickness), colPrimary(colPrimary), colBackground(colBackground), colOutline(colOutline), bBackground(background), bOutline(outline) { }
		...
		[[nodiscard]] const T* GetStorage() const
		{
			...
			if constexpr (bTypeSafe)
			{
				if (const std::type_info& currentTypeInfo = typeid(std::remove_cvref_t<T>); this->pTypeInfo != nullptr && CRT::StringCompare(this->pTypeInfo->raw_name(), currentTypeInfo.raw_name()) != 0)
				{
					...
					{
						L_PRINT(LOG_ERROR) << CXOR("accessing variable of type: \"") << szPresentTypeName << CXOR("\" with wrong type: \"") << szAccessTypeName << CXOR("\"");
					}
