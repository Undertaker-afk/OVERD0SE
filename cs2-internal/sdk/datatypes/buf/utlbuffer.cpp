	...
	m_Memory(reinterpret_cast<const unsigned char*>(pBuffer), static_cast<int>(nSize)), m_Error(0)
{
	...
	if (IsReadOnly())
	{	
		printf(CXOR("CUTLBuffer > readonly\n"));
		m_nMaxPut = m_Put = nSize;
	}
	else
	{
		printf(CXOR("CUTLBuffer > m_MaxPut = -1\n"));
...
inline 	bool(CS_FASTCALL* LoadKV3)(KeyValues3*, void*, const char*, const KV3ID_t*, const char*) = nullptr;

bool CS_FASTCALL KeyValues3::loadkv3(KeyValues3* kv3,const char* buf,   KV3ID_t kvid)
{
	const void* hTier0 = MEM::GetModuleBaseHandle(TIER0_DLL);

	LoadKV3 = reinterpret_cast<decltype(LoadKV3)>(MEM::GetExportAddress(hTier0, CXOR("?LoadKV3@@YA_NPEAVKeyValues3@@PEAVCUtlString@@PEBDAEBUKV3ID_t@@2@Z")));
...
{
	fnSetType = reinterpret_cast<decltype(fnSetType)>(MEM::FindPattern(CLIENT_DLL, CXOR("48 8B C4 55 48 8D A8 08 FD")));
	CS_ASSERT(fnSetType != nullptr);
	return fnSetType(a1, a2);
}
inline 	bool(CS_FASTCALL* fnLoadfromBuffer)(KeyValues3*, const char*, CUtlBuffer*, void*, const char*, bool(_stdcall*)(const char*, void*), void*, void*, const char*, bool) = nullptr;
inline 	bool(CS_FASTCALL* fnLoadfromBufferchar)(KeyValues3*, const char*, const char*, void*, const char*, bool(_stdcall*)(const char*, void*), void*, void*, const char*, bool) = nullptr;

bool KeyValues3::load_from_buffer_char(const char* buf)
{
	const void* hTier0 = MEM::GetModuleBaseHandle(TIER0_DLL);

	fnLoadfromBufferchar = reinterpret_cast<decltype(fnLoadfromBufferchar)>(MEM::GetExportAddress(hTier0, CXOR("?LoadFromBuffer@KeyValues@@QEAA_NPEBD0PEAVIFileSystem@@0P6A_N0PEA")));
...
bool KeyValues3::load_from_buffer(CUtlBuffer* buf)
{
	...
	fnLoadfromBuffer = reinterpret_cast<decltype(fnLoadfromBuffer)>(MEM::GetExportAddress(hTier0, CXOR("?LoadFromBuffer@KeyValues@@QEAA_NPEBDAEAVCUtlBuffer@@PEAVIFileSystem@@0P6A_N0PEAX@ZPEAVIKeyValuesErrorSpew@@30@Z")));
...
bool KeyValues3::load_kv3(CUtlBuffer* buf, const char* material, KV3ID_t kvid)
{
	...
	LoadKv3 = reinterpret_cast<decltype(LoadKv3)>(MEM::GetExportAddress(hTier0, CXOR("?LoadKV3@@YA_NPEAVKeyValues3@@PEAVCUtlString@@PEAVCUtlBuffer@@AEBUKV3ID_t@@PEBD@Z")));
	CS_ASSERT(LoadKv3 != nullptr);
...
inline void(CS_FASTCALL* utlbuf)(c_ult_buffer*, int, int, int) = nullptr;
inline void(CS_FASTCALL* putstring)(c_ult_buffer*, const char*) = nullptr;
inline c_key_values*(CS_FASTCALL* settype)(c_key_values*, std::uint8_t, std::uint8_t) = nullptr;

c_ult_buffer::c_ult_buffer(int a1, int size, int a3) {


	utlbuf = reinterpret_cast<decltype(utlbuf)>(MEM::FindPattern(TIER0_DLL, CXOR("48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 48 89 7C 24 ?? 41 56 48 83 EC 20 33 DB 44 89 41 08")));
	CS_ASSERT(utlbuf != nullptr);
	utlbuf(this, a1, size, a3);
}

void c_ult_buffer::put_string(std::string_view string) {
	putstring = reinterpret_cast<decltype(putstring)>(MEM::ResolveRelativeAddress(MEM::FindPattern(TIER0_DLL, CXOR("E8 ?? ?? ?? ?? EB 57")), 1, 0));
...
c_key_values* create_material_resource() {
	...
	settype = reinterpret_cast<decltype(settype)>(MEM::ResolveRelativeAddress(MEM::FindPattern(CLIENT_DLL, CXOR("E8 ?? ?? ?? ?? 66 85 DB")), 0x1, 0x0));
	CS_ASSERT(settype != nullptr);
	return settype(kv, 1, 6);
}
c_key_values* c_key_values::set_type()
{
	settype = reinterpret_cast<decltype(settype)>(MEM::FindPattern(CLIENT_DLL, CXOR("48 89 5C 24 08 57 48 83 EC 30 48 0F")));
	CS_ASSERT(settype != nullptr);
	return settype(this, 1, 6);
}
bool c_key_values::load_kv3(c_ult_buffer* buffer) {

	LoadKv3 = reinterpret_cast<decltype(LoadKv3)>(MEM::ResolveRelativeAddress(MEM::FindPattern(TIER0_DLL, CXOR("E8 ?? ?? ?? ?? EB 36 8B 43 10")), 0x1, 0x0));
	CS_ASSERT(LoadKv3 != nullptr);
