...
public:
    const char* insert(int index, const char* buf, int count = -1, bool ignore_alignment = false) {
        using fn = const char* (CS_FASTCALL*)(void*, int, const char*, int, bool);
        static auto func = reinterpret_cast< fn >(MEM::FindPattern(TIER0_DLL, CXOR("40 53 56 57 41 57 48 83 EC ? 4C 63 FA")));

        return func(this, index, buf, count, ignore_alignment);
    }

    void purge(int allocated_bytes_to_preserve = 0) {
        using fn = void(CS_FASTCALL*)(void*, int);
        static auto func = reinterpret_cast< fn >(MEM::FindPattern(TIER0_DLL, CXOR("48 89 5C 24 ? 57 48 83 EC ? 8B 41 ? 8D 7A")));
